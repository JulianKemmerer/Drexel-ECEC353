...
//Compile and run with gcc FourInAQueue.c csapp.c -pthread -o FourInAQueue && ./FourInAQueue

#define GRID_N  5 // NxN grid exists
#define M_CHILDREN  4 // Number of children games to spawn
...
{ 
  //Structs to hold receive info
  struct {
    int column;
  } msgbuffr;
  
  //Receive the message
  if (msgrcv(readFromID, &msgbuffr, sizeof(msgbuffr), 0, 0) != sizeof(msgbuffr))
  {
    //perror("msgrcv error");
  }

  //Print received message
   //printf("the received column from %d is: %d \n", childrenPIDs[childIndexFromActiveBoard()],  msgbuffr.column);

   //Remove msg
    if (msgctl(readFromID, IPC_RMID, (struct msqid_ds *) 0) < 0)
    {
	//perror("IPC_RMID error");
    }
    
    return msgbuffr.column;
}
...
void sendMove(int toID, int column)
{ 
  //Structs to hold send info
  struct {
    int column;
  } msgbuffs;
    
  //Copy in message
  msgbuffs.column=column;
  
  //Send the message
  if (msgsnd(toID, &msgbuffs, sizeof(msgbuffs), 0) < 0)
  {
    //perror("msgsnd error");
  }
  
  //Print sent message
  //printf("the column sent from %d is: %d \n", getpid(), msgbuffs.column);  
}
...
{  
  ...
  //Unlike pipes, reading and writing can be done on both ends of a single queue
  *toChildWrite = msqid;
  *fromChildRead = msqid;
  *toParentWrite = msqid;
  *fromParentRead = msqid;
}
...
{  
  //Child does dumbplay
  //Child does not check for winning
  int columnPlayed = dumbPlay(CHILD_PIECE);
